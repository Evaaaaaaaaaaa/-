...
* **公式：**IFERROR(需要检查的公式，如果公式错误展示的信息)
* **举例：**IFERROR(SUM(A1:A3),"无法相加")，可以把结果写入A4
* **目的**：正常来说公式错误后储存格回被写入"#N/A"，IFERROR在这种情况时帮助表格写入有意义的值
* **注意**：**IFERROR()通常于IF()函数一起使用**
  * **原因：**以上述例子为例，在公式内中的取值A1:A5为空白时(未填写时)，当前储存格也会被写入“无法相加”。
  * **解决方法：**=IF(AND(A1="", A2="", A3=""), "",IFERROR(...)) 
    * 当A1:A3所有储存格值都为空白时，A4 = "", 有一个不是空白时，采用IFERROR()
...
* 方法：
  1. 选择验证的储存格位置
  2. 点击工具栏内“资料” - “资料验证”
  3. 输入此储存格的数据要求，提示语，错误警告信息等

### COUNT函数

* **目的**: 计算选中表/列的行数
* 注意：只能计算数字，且**不会**计算NULL的储存格



### COUNTA函数

* 目的：同COUNT(), 计算选中表/列的行数, 但是可以中文英文列，且**不会**计算NULL的储存格